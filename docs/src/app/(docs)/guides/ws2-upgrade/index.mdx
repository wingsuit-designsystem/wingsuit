...
Upgrading `Wingsuit 1` to `Wingsuit 2` also upgrades `Storybook 6` to `Storybook 7` and `Webpack 4` to `Webpack 5`.
...
## 1. Upgrade to Node 18 or higher
The easiest way to upgrade to node 18 or higher is nvm. Install nvm and run nvm install 18 and nvm use 18
